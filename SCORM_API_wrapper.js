...
/* -------------------------------------------------------------------------
 ...
 Parameters: value (string). The value for the data model element
 Returns:    Boolean
 ---------------------------------------------------------------------------- */

pipwerks.SCORM.setByVersion = function (parameter12, parameter13, value) {
  ...
  if (scorm.connection.isActive) {
    ...
    if (API) {
      ...
      switch (scorm.version) {
        case "1.2" :
          success = makeBoolean(API.LMSSetValue(parameter12, value));
          break;
        case "2004":
          success = makeBoolean(API.SetValue(parameter13, value));
          break;
      }

      if (success) {

        if (parameter12 === "cmi.core.lesson_status" || parameter13 === "cmi.completion_status") {
